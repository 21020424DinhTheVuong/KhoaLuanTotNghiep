...
function EditPost() {
    ...
    const { postId, replyPostId, data } = route.params;

    const [contentPost, setContentPost] = useState(data.content)

    const [selectedMedia, setSelectedMedia] = useState<any[]>(
        data.content_image
            ? data.content_image.split(",").map((img: any) => ({ uri: `${baseURL}/${img}`, name: img, type: img.split(".").pop() === "mp4" ? "video/mp4" : "image/jpeg" }))
            : []
    );
    // console.log(selectedMedia)
    const [hasChangeMedia, setHasChangeMedia] = useState(false)
    ...
    const updatePostOrReply = async () => {
        const formData = new FormData();
        formData.append("content", contentPost);

        if (hasChangeMedia && selectedMedia && selectedMedia.length > 0) {
            selectedMedia.forEach((media) => {
                formData.append(postId ? "content_media" : "reply_post", {
                    uri: media.uri,
        ...
        try {
            await apiClient.patch(`posts/${postId ? `update-post/${postId}` : `update-reply-post/${replyPostId}`}`, formData, {
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{
                ...
                <TouchableOpacity onPress={() => { updatePostOrReply() }}
                    disabled={contentPost.trim() === ''}
                    style={{ opacity: contentPost.trim() === '' ? 0.6 : 1 }}
                >
