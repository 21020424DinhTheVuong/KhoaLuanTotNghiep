...
const ReplyChildren = ({ children, action, refreshAfterDelete }: ReplyInterface) => {
    const { user } = useAuth()
        ...
        <View style={styles.container}>
            <View style={styles.avatarContainer}>
                {
                    children.user.avatar ?
                        <Image source={{ uri: `${baseURL}/${children.user.avatar}` }} style={styles.avatar} />
                        :
            ...
            <View style={styles.postContainer}>
                <View style={styles.usernameContainer}>
                    <Text style={{ fontSize: 17, fontWeight: 700 }}>{children.user.display_name}</Text>

                    <View style={styles.actionPost}>
                        <Text style={{ color: "gray", fontSize: 14 }}>{formatTimeAgo(children.update_at)}</Text>
                            ...
                            visibleModal && (
                                ...
                                >
                                    <TouchableOpacity onPress={() => { handleShowReportModal() }}>
                                        <ReasonReport reportType={user?.id === children.user.id ? "userReply" : "report"}
                                            replyPostChildrenId={children.id}
                                            reportedId={children.user.id}
                                            refresh={refreshAfterDelete}
                                            onCloseReportModal={() => { setVisibleModal(false) }}
                                            data={{ id: children.id, content: children.content_reply_post_children }} />
                                    </TouchableOpacity>
                ...
                <View style={styles.contentContainer}>
                    <Text>{children.content_reply_post_children}</Text>
                </View>
